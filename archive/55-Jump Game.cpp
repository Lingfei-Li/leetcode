...
bool canJump(vector<int>& nums) {
    ...
    int u = 0, e = nums[0];
    while(1) {
        e = max(e, u+nums[u]);      //update maximum length of jump.
        if(u >= e || u >= nums.size()) break;       //if u has reached the end of farthest jump area, stop.
        u ++;
    }
//    cout<<u<<endl;
    if(u < nums.size() - 1) return false;
    return true;
}
...
//	freopen("55-Jump Game.in","r",stdin);
